...
@interface KSPageView () <UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout>

@property (nonatomic, strong) KSPageCollectionView *collectionView;
...
- (void)reloadCellAtIndex:(NSInteger)index {
    ...
    KSPageViewCellModel *cellModel = self.dataSource[index];
    cellModel.selectedType = KSPageViewCellSelectedTypeUnknown;
    [self refreshCellModel:cellModel index:index];
    KSPageViewCell *cell = (KSPageViewCell *)[self.collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForItem:index inSection:0]];
    [cell reloadData:cellModel];
}

- (void)selectItemAtIndex:(NSInteger)index {
    [self selectCellAtIndex:index selectedType:KSPageViewCellSelectedTypeCode];
}
...
- (void)layoutSubviews
{
    ...
    //部分使用者为了适配不同的手机屏幕尺寸，KSPageView的宽高比要求保持一样，所以它的高度就会因为不同宽度的屏幕而不一样。计算出来的高度，有时候会是位数很长的浮点数，如果把这个高度设置给UICollectionView就会触发内部的一个错误。所以，为了规避这个问题，在这里对高度统一向下取整。
    //如果向下取整导致了你的页面异常，请自己重新设置KSPageView的高度，保证为整数即可。
...
- (void)setDelegate:(id<KSPageViewDelegate>)delegate {
    _delegate = delegate;

    _delegateFlags.didSelectedItemAtIndexFlag = [delegate respondsToSelector:@selector(pageView:didSelectedItemAtIndex:)];
    _delegateFlags.didClickSelectedItemAtIndexFlag = [delegate respondsToSelector:@selector(pageView:didClickSelectedItemAtIndex:)];
    _delegateFlags.didScrollSelectedItemAtIndexFlag = [delegate respondsToSelector:@selector(pageView:didScrollSelectedItemAtIndex:)];
    _delegateFlags.canClickItemAtIndexFlag = [delegate respondsToSelector:@selector(pageView:canClickItemAtIndex:)];
    _delegateFlags.scrollingFromLeftIndexToRightIndexFlag = [delegate respondsToSelector:@selector(pageView:scrollingFromLeftIndex:toRightIndex:ratio:)];
}
...
- (void)setListContainer:(id<KSPageViewListContainer>)listContainer {
...
- (void)collectionView:(UICollectionView *)collectionView willDisplayCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath {
    KSPageViewCellModel *cellModel = self.dataSource[indexPath.item];
    cellModel.selectedType = KSPageViewCellSelectedTypeUnknown;
    [(KSPageViewCell *)cell reloadData:cellModel];
}

- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    BOOL isTransitionAnimating = NO;
    for (KSPageViewCellModel *cellModel in self.dataSource) {
...
- (CGFloat)getContentEdgeInsetLeft {
    if (self.contentEdgeInsetLeft == KSPageViewAutomaticDimension) {
        return self.innerCellSpacing;
    }
...
- (CGFloat)getContentEdgeInsetRight {
    if (self.contentEdgeInsetRight == KSPageViewAutomaticDimension) {
        return self.innerCellSpacing;
    }
...
- (void)clickSelectItemAtIndex:(NSInteger)index {
    if (self.delegateFlags.canClickItemAtIndexFlag && ![self.delegate pageView:self canClickItemAtIndex:index]) {
        return;
    }

    [self selectCellAtIndex:index selectedType:KSPageViewCellSelectedTypeClick];
}

- (void)scrollSelectItemAtIndex:(NSInteger)index {
    [self selectCellAtIndex:index selectedType:KSPageViewCellSelectedTypeScroll];
}
...
@implementation KSPageView (UISubclassingHooks)

- (CGRect)getTargetCellFrame:(NSInteger)targetIndex
{
    CGFloat x = [self getContentEdgeInsetLeft];
    for (int i = 0; i < targetIndex; i ++) {
        KSPageViewCellModel *cellModel = self.dataSource[i];
        CGFloat cellWidth;
    ...
    CGFloat width;
    KSPageViewCellModel *selectedCellModel = self.dataSource[targetIndex];
...
- (CGRect)getTargetSelectedCellFrame:(NSInteger)targetIndex selectedType:(KSPageViewCellSelectedType)selectedType
{
    CGFloat x = [self getContentEdgeInsetLeft];
    for (int i = 0; i < targetIndex; i ++) {
        KSPageViewCellModel *cellModel = self.dataSource[i];
...
- (void)initializeData
{
    ...
    _selectedIndex = 0;
    _cellWidth = KSPageViewAutomaticDimension;
    ...
    _contentEdgeInsetLeft = KSPageViewAutomaticDimension;
    _contentEdgeInsetRight = KSPageViewAutomaticDimension;
...
- (void)initializeViews
{
    ...
    _collectionView = [[KSPageCollectionView alloc] initWithFrame:self.bounds collectionViewLayout:layout];
...
- (void)refreshState {
    ...
    if (self.isAverageCellSpacingEnabled && totalItemWidth < self.bounds.size.width) {
        ...
        [self.dataSource enumerateObjectsUsingBlock:^(KSPageViewCellModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
    ...
    [self.dataSource enumerateObjectsUsingBlock:^(KSPageViewCellModel * cellModel, NSUInteger idx, BOOL * _Nonnull stop) {
    ...
    if (CGRectEqualToRect(self.contentScrollView.frame, CGRectZero) && self.contentScrollView.superview != nil) {
        //某些情况系统会出现KSPageView先布局，contentScrollView后布局。就会导致下面指定defaultSelectedIndex失效，所以发现contentScrollView的frame为zero时，强行触发其父视图链里面已经有frame的一个父视图的layoutSubviews方法。
...
- (BOOL)selectCellAtIndex:(NSInteger)targetIndex selectedType:(KSPageViewCellSelectedType)selectedType {
    ...
    if (self.selectedIndex == targetIndex) {
        ...
        if (selectedType == KSPageViewCellSelectedTypeCode) {
            [self.listContainer didClickSelectedItemAtIndex:targetIndex];
        }else if (selectedType == KSPageViewCellSelectedTypeClick) {
            [self.listContainer didClickSelectedItemAtIndex:targetIndex];
            if (self.delegateFlags.didClickSelectedItemAtIndexFlag) {
                [self.delegate pageView:self didClickSelectedItemAtIndex:targetIndex];
            }
        }else if (selectedType == KSPageViewCellSelectedTypeScroll) {
            if (self.delegateFlags.didScrollSelectedItemAtIndexFlag) {
                [self.delegate pageView:self didScrollSelectedItemAtIndex:targetIndex];
            }
        }
        if (self.delegateFlags.didSelectedItemAtIndexFlag) {
            [self.delegate pageView:self didSelectedItemAtIndex:targetIndex];
        }
    ...
    KSPageViewCellModel *lastCellModel = self.dataSource[self.selectedIndex];
    ...
    //刷新当前选中的和将要选中的cell
    KSPageViewCell *lastCell = (KSPageViewCell *)[self.collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForItem:self.selectedIndex inSection:0]];
    [lastCell reloadData:lastCellModel];
    KSPageViewCell *selectedCell = (KSPageViewCell *)[self.collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForItem:targetIndex inSection:0]];
    [selectedCell reloadData:selectedCellModel];

    if (self.scrollingTargetIndex != -1 && self.scrollingTargetIndex != targetIndex) {
        KSPageViewCellModel *scrollingTargetCellModel = self.dataSource[self.scrollingTargetIndex];
        ...
        KSPageViewCell *scrollingTargetCell = (KSPageViewCell *)[self.collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForItem:self.scrollingTargetIndex inSection:0]];
    ...
    if (selectedType == KSPageViewCellSelectedTypeClick ||
        selectedType == KSPageViewCellSelectedTypeCode) {
...
- (void)refreshSelectedCellModel:(KSPageViewCellModel *)selectedCellModel unselectedCellModel:(KSPageViewCellModel *)unselectedCellModel {
    ...
    if (self.isCellWidthZoomEnabled) {
        if (selectedCellModel.selectedType == KSPageViewCellSelectedTypeCode ||
            selectedCellModel.selectedType == KSPageViewCellSelectedTypeClick) {
            ...
            self.animator.progressCallback = ^(CGFloat percent) {
                ...
                selectedCellModel.cellWidthCurrentZoomScale = [KSPageViewUtil interpolationFrom:selectedCellModel.cellWidthNormalZoomScale to:selectedCellModel.cellWidthSelectedZoomScale percent:percent];
                ...
                unselectedCellModel.cellWidthCurrentZoomScale = [KSPageViewUtil interpolationFrom:unselectedCellModel.cellWidthSelectedZoomScale to:unselectedCellModel.cellWidthNormalZoomScale percent:percent];
...
- (void)contentOffsetOfContentScrollViewDidChanged:(CGPoint)contentOffset {
    ...
    CGFloat remainderRatio = ratio - Index;
    ...
    if (remainderRatio == 0) {
        ...
        if (!(self.lastContentViewContentOffset.x == contentOffset.x && self.selectedIndex == Index)) {
            [self scrollSelectItemAtIndex:Index];
        }
    }else {
        ...
        if (self.animator.isExecuting) {
            ...
            for (KSPageViewCellModel *model in self.dataSource) {
                if (model.isSelected) {
        ...
        if (self.selectedIndex == Index) {
            self.scrollingTargetIndex = Index + 1;
        }else {
            self.scrollingTargetIndex = Index;
        }

        if (self.isCellWidthZoomEnabled && self.isCellWidthZoomScrollGradientEnabled) {
            KSPageViewCellModel *leftCellModel = (KSPageViewCellModel *)self.dataSource[Index];
            KSPageViewCellModel *rightCellModel = (KSPageViewCellModel *)self.dataSource[Index + 1];
            leftCellModel.cellWidthCurrentZoomScale = [KSPageViewUtil interpolationFrom:leftCellModel.cellWidthSelectedZoomScale to:leftCellModel.cellWidthNormalZoomScale percent:remainderRatio];
            leftCellModel.cellWidth = [self getCellWidthAtIndex:leftCellModel.index] * leftCellModel.cellWidthCurrentZoomScale;
            rightCellModel.cellWidthCurrentZoomScale = [KSPageViewUtil interpolationFrom:rightCellModel.cellWidthNormalZoomScale to:rightCellModel.cellWidthSelectedZoomScale percent:remainderRatio];
        ...
        if (self.delegateFlags.scrollingFromLeftIndexToRightIndexFlag) {
            [self.delegate pageView:self scrollingFromLeftIndex:Index toRightIndex:Index + 1 ratio:remainderRatio];
        }
    }
}
