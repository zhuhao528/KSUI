...
@protocol KSPageViewListContainer <NSObject>
...
@protocol KSPageViewDelegate <NSObject>
 ...
 */
- (void)pageView:(KSPageView *)pageView didSelectedItemAtIndex:(NSInteger)index;
...
/**
 ...
 @param index 选中的index
 */
- (void)pageView:(KSPageView *)pageView didClickSelectedItemAtIndex:(NSInteger)index;
...
/**
 ...
 @param index 选中的index
 */
- (void)pageView:(KSPageView *)pageView didScrollSelectedItemAtIndex:(NSInteger)index;
...
/**
 ...
 @return 能否点击
 */
- (BOOL)pageView:(KSPageView *)pageView canClickItemAtIndex:(NSInteger)index;
 ...
 */
- (void)pageView:(KSPageView *)pageView scrollingFromLeftIndex:(NSInteger)leftIndex toRightIndex:(NSInteger)rightIndex ratio:(CGFloat)ratio;

@end

@interface KSPageView : UIView

@property (nonatomic, strong, readonly) KSPageCollectionView *collectionView;

@property (nonatomic, strong) NSArray <KSPageViewCellModel *> *dataSource;

@property (nonatomic, weak) id<KSPageViewDelegate> delegate;

/**
 高封装度的列表容器，使用该类可以让列表拥有完成的生命周期、自动同步defaultSelectedIndex、自动调用reloadData。
 */
@property (nonatomic, weak) id<KSPageViewListContainer> listContainer;
...
@property (nonatomic, assign) CGFloat contentEdgeInsetLeft;     //整体内容的左边距，默认KSPageViewAutomaticDimension（等于cellSpacing）

@property (nonatomic, assign) CGFloat contentEdgeInsetRight;    //整体内容的右边距，默认KSPageViewAutomaticDimension（等于cellSpacing）

@property (nonatomic, assign) CGFloat cellWidth;    //默认KSPageViewAutomaticDimension
...
@property (nonatomic, assign) CGFloat cellRadius;    //cell的圆角

@property (nonatomic, assign, getter=isAverageCellSpacingEnabled) BOOL averageCellSpacingEnabled;     //当collectionView.contentSize.width小于KSPageView的宽度，是否将cellSpacing均分。默认为YES。
...
/**
 初始化的时候无需调用。比如页面初始化之后，根据网络接口异步回调回来数据，重新配置pageView，需要调用该方法进行刷新。
 */
- (void)reloadData;

/**
 重新配置pageView但是不需要reload listContainer。特殊情况是该方法。
 */
- (void)reloadDataWithoutListContainer;

/**
 刷新指定的index的cell
 内部会触发`- (void)refreshCellModel:(KSPageViewCellModel *)cellModel index:(NSInteger)index`方法进行cellModel刷新
...
@interface KSPageView (UISubclassingHooks)
 ...
 */
- (void)refreshSelectedCellModel:(KSPageViewCellModel *)selectedCellModel unselectedCellModel:(KSPageViewCellModel *)unselectedCellModel NS_REQUIRES_SUPER;
...
/**
 ...
 @param index 选中的index
 @param selectedType KSPageCellSelectedType
 @return 返回值为NO，表示触发内部某些判断（点击了同一个cell），子类无需后续操作。
 */
- (BOOL)selectCellAtIndex:(NSInteger)index selectedType:(KSPageViewCellSelectedType)selectedType NS_REQUIRES_SUPER;
 ...
 */
- (void)refreshCellModel:(KSPageViewCellModel *)cellModel index:(NSInteger)index NS_REQUIRES_SUPER;
