...
#import "KSPageTitleView.h"
#import "KSPageViewUtil.h"

@interface KSPageTitleView ()

@end

@implementation KSPageTitleView
...
- (void)initializeData
{
    ...
    _titleLabelAnchorPointStyle = KSPageViewTitleLabelAnchorPointStyleCenter;
}
...
- (Class)preferredCellClass {
    return [KSPageTitleViewCell class];
}

- (void)refreshDataSource {
    NSMutableArray *tempArray = [NSMutableArray array];
    for (int i = 0; i < self.titles.count; i++) {
        KSPageTitleViewCellModel *cellModel = [[KSPageTitleViewCellModel alloc] init];
...
- (void)refreshSelectedCellModel:(KSPageViewCellModel *)selectedCellModel unselectedCellModel:(KSPageViewCellModel *)unselectedCellModel {
    ...
    KSPageTitleViewCellModel *myUnselectedCellModel = (KSPageTitleViewCellModel *)unselectedCellModel;
    KSPageTitleViewCellModel *myselectedCellModel = (KSPageTitleViewCellModel *)selectedCellModel;
    if (self.isSelectedAnimationEnabled && (selectedCellModel.selectedType == KSPageViewCellSelectedTypeClick || selectedCellModel.selectedType == KSPageViewCellSelectedTypeCode)) {
...
- (void)refreshLeftCellModel:(KSPageViewCellModel *)leftCellModel rightCellModel:(KSPageViewCellModel *)rightCellModel ratio:(CGFloat)ratio {
    ...
    KSPageTitleViewCellModel *leftModel = (KSPageTitleViewCellModel *)leftCellModel;
    KSPageTitleViewCellModel *rightModel = (KSPageTitleViewCellModel *)rightCellModel;

    if (self.isTitleLabelZoomEnabled && self.isTitleLabelZoomScrollGradientEnabled) {
        leftModel.titleLabelCurrentZoomScale = [KSPageViewUtil interpolationFrom:self.titleLabelZoomScale to:1.0 percent:ratio];
        rightModel.titleLabelCurrentZoomScale = [KSPageViewUtil interpolationFrom:1.0 to:self.titleLabelZoomScale percent:ratio];
    }

    if (self.isTitleLabelStrokeWidthEnabled) {
        leftModel.titleLabelCurrentStrokeWidth = [KSPageViewUtil interpolationFrom:leftModel.titleLabelSelectedStrokeWidth to:leftModel.titleLabelNormalStrokeWidth percent:ratio];
        rightModel.titleLabelCurrentStrokeWidth = [KSPageViewUtil interpolationFrom:rightModel.titleLabelNormalStrokeWidth to:rightModel.titleLabelSelectedStrokeWidth percent:ratio];
    }

    if (self.isTitleColorGradientEnabled) {
        leftModel.titleCurrentColor = [KSPageViewUtil interpolationColorFrom:self.titleSelectedColor to:self.titleColor percent:ratio];
        rightModel.titleCurrentColor = [KSPageViewUtil interpolationColorFrom:self.titleColor to:self.titleSelectedColor percent:ratio];
    }
}
...
- (void)refreshCellModel:(KSPageViewCellModel *)cellModel index:(NSInteger)index {
    ...
    KSPageTitleViewCellModel *model = (KSPageTitleViewCellModel *)cellModel;
