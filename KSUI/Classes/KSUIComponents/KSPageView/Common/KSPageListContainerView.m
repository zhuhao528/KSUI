...
@interface KSPageListContainerViewController : UIViewController
...
@implementation KSPageListContainerViewController
- (void)dealloc
{
...
@interface KSPageListContainerView () <UIScrollViewDelegate, UICollectionViewDelegate, UICollectionViewDataSource>
@property (nonatomic, weak) id<KSPageListContainerViewDelegate> delegate;
@property (nonatomic, strong) UIScrollView *scrollView;
@property (nonatomic, assign) NSInteger currentIndex;
@property (nonatomic, strong) NSMutableDictionary <NSNumber *, id<KSPageListContentViewDelegate>> *validListDict;
@property (nonatomic, assign) NSInteger willAppearIndex;
@property (nonatomic, assign) NSInteger willDisappearIndex;
@property (nonatomic, strong) UICollectionView *collectionView;
@property (nonatomic, strong) KSPageListContainerViewController *containerVC;
@end

@implementation KSPageListContainerView

- (instancetype)initWithType:(KSPageListContainerType)type delegate:(id<KSPageListContainerViewDelegate>)delegate{
...
- (void)initializeViews {
    ...
    _containerVC = [[KSPageListContainerViewController alloc] init];
    ...
    if (self.containerType == KSPageListContainerType_ScrollView) {
        if (self.delegate &&
...
- (void)layoutSubviews {
    ...
    if (self.containerType == KSPageListContainerType_ScrollView) {
        if (CGRectEqualToRect(self.scrollView.frame, CGRectZero) ||  !CGSizeEqualToSize(self.scrollView.bounds.size, self.bounds.size)) {
            ...
            [_validListDict enumerateKeysAndObjectsUsingBlock:^(NSNumber * _Nonnull index, id<KSPageListContentViewDelegate>  _Nonnull list, BOOL * _Nonnull stop) {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    id<KSPageListContentViewDelegate> list = _validListDict[@(indexPath.item)];
    if (list != nil) {
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(listContainerView:endScrollAtIndex:)]) {
        [self.delegate listContainerView:self endScrollAtIndex:self.currentIndex];
    }
}
...
#pragma mark - KSPageViewListContainer
...
- (void)reloadData {
    for (id<KSPageListContentViewDelegate> list in _validListDict.allValues) {
...
- (void)initListIfNeededAtIndex:(NSInteger)index {
    ...
    id<KSPageListContentViewDelegate> list = _validListDict[@(index)];
    if (list != nil) {
        //列表已经创建好了
        return;
    }
