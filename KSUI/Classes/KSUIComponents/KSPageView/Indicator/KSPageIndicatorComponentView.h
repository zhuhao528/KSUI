//
//  KSPageIndicatorComponentView.h
//  KSUI
//
...
@interface KSPageIndicatorComponentView : UIView<KSPageIndicatorProtocol>

/**
 指示器的位置。底部或者顶部
 */
@property (nonatomic, assign) KSPageViewComponentPosition componentPosition;

/**
 默认KSPageViewAutomaticDimension（与cell的宽度相等）。内部通过`- (CGFloat)indicatorWidthValue:(CGRect)cellFrame`方法获取实际的值
 */
...
/**
 默认KSPageViewAutomaticDimension （等于indicatorHeight/2）。内部通过`- (CGFloat)indicatorCornerRadiusValue:(CGRect)cellFrame`方法获取实际的值
 */
...
/**
 手势滚动、点击切换的时候，如果允许滚动，分为简单滚动和复杂滚动。默认为：KSPageIndicatorScrollStyleSimple
 目前仅KSPageIndicatorLineView、KSPageIndicatorDotLineView支持，其他子类暂不支持。
 */
@property (nonatomic, assign) KSPageViewIndicatorScrollStyle scrollStyle;
